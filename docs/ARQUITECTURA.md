...
│  │ reports.ts  │  │  sessions    │  │  AI (openai|none)     │   │
│  │ admin.ts    │  │  (memory)◄───│──│  WhatsApp (generic)   │   │
│  │ external.ts │  │  JWT (futuro)│  │  Ingestion plugins    │   │
│  │ whatsapp.ts │  └──────────────┘  └──────────────────────┘   │
│  │ orgs.ts     │                       │         │             │
│  └─────┬───────┘                       ▼         ▼             │
│        │                     ┌────────────┐ ┌───────────┐      │
...
| **Sesiones**         | `Map` en memoria            | JWT + Redis (multi-instancia)                    | `auth.ts` — exporta la misma interfaz                                    |
