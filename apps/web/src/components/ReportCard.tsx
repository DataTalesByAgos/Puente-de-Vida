    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 border-t border-line pt-2">
          <span className={`chip ${STATUS_CHIP[report.status]}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="Compartir con brigadista en terreno"
            >
              {showQR ? '✕' : '📲 QR'}
            </button>
          </div>
        ...
        {showQR && (
          <div
            className="flex flex-col items-center gap-2 border-t border-line pt-3"
            onClick={(e) => e.stopPropagation()}
          >
            <p className="text-[11px] text-muted text-center max-w-xs">
              El brigadista escanea este código y carga el reporte en su móvil para atenderlo en
              terreno.
            </p>
            <QRCode
      ...
      {showTriage && next && (
        <TriageModal report={report} next={next} onClose={() => setShowTriage(false)} />
      )}
    </>
  );
}
